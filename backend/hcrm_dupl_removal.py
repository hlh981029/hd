from commons import colormap, timeit, get_model, get_torch_model, get_transform, init_position, max_crop, \
    Timer, pickle_dump, pickle_load, save_cam, activation_sort, activation_max_sort, save_crop_cam, dupl_removal
...
def get_kernel_images_5(filename, label):
    ...
    gb_feature_maps, gb_logits = gb_model.forward_cam(
        input_var1, target_layers1)
    ...
    #print(avg_sort)
    keep_inds, keep_avg_sort = dupl_removal(pre_cam, inds, avg_sort)
    top_n = min(len(keep_inds), top_n)
    ...
    for j in range(top_n):
        ...
        assert unit_label.nonzero(as_tuple=False).shape[0] == 1
        indices = unit_label[0, ind].nonzero(as_tuple=False).tolist()
        ...
        origin_image = Image.fromarray(
            np.asarray((crop_grad * 255).astype(np.uint8)))
...
def get_kernel_images_4(filename, label, index):
    ...
    #print(avg_sort)
    keep_inds, keep_avg_sort = dupl_removal(conv4_cam, inds, avg_sort)
    top_n = min(len(keep_inds), top_n)
...
def get_kernel_images_3(filename, label, index_5, index_4):
    ...
    #print(avg_sort)
    keep_inds, keep_avg_sort = dupl_removal(conv3_cam, inds, avg_sort)
    top_n = min(len(keep_inds), top_n)
...
def get_kernel_images_2(filename, label, index_5, index_4, index_3):
    ...
    #print(avg_sort)
    keep_inds, keep_avg_sort = dupl_removal(conv2_cam, inds, avg_sort)
    top_n = min(len(keep_inds), top_n)
...
def get_kernel_images_1(filename, label, index_5, index_4, index_3, index_2):
    ...
    unit_label[0][tuple(position[index_5][index_4]
                        [index_3][9][index_2][:3])] = 1
    ...
    #print(avg_sort)
    keep_inds, keep_avg_sort = dupl_removal(conv1_cam, inds, avg_sort)
    top_n = min(len(keep_inds), top_n)
