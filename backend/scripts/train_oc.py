...
def get_arguments():
    ...
    parser.add_argument("--decay_points", type=str, default='30, 45')
...
def get_model(args):
    ...
    #optimizer = optim.SGD(param_groups, momentum=0.9, weight_decay=args.weight_decay, nesterov=True)
    if args.resume == 'True':
...
def validate(model, val_loader):
    ...
    with torch.no_grad():
        for idx, dat in tqdm(enumerate(val_loader)):
            ...
            label = label.long()
            logits = model(img)
            loss_val = F.cross_entropy(logits, label)   
...
def train(args):
    ...
    #param_groups = model.module.get_parameter_groups()
    #optimizer = my_optim.PolyOptimizer([
    #    {'params': param_groups[0], 'lr': args.lr},
    #    {'params': param_groups[1], 'lr': 2*args.lr},
    #    {'params': param_groups[2], 'lr': 10*args.lr},
    #    {'params': param_groups[3], 'lr': 20*args.lr}], lr=args.lr, weight_decay=args.weight_decay, max_step=max_step)
    ...
    while current_epoch < total_epoch:
        ...
        writer.add_scalar('training loss', train_loss, current_epoch)
        writer.add_scalar('testing loss', val_loss, current_epoch)
        current_epoch += 1
